import { LayoutDashboard, UtensilsCrossed, Users, ShoppingBag, BarChart2, Star, Settings, LogOut, ChevronLeft, Menu,BikeIcon } from "lucide-react";

const navItems = [
  { id: "overview", label: "Overview", icon: LayoutDashboard },
  { id: "vendor", label: "Vendor Management", icon: UtensilsCrossed },
  { id: "customer", label: "Customer Management", icon: Users },
  { id: "deliveryman", label: "Deliveryman Management", icon: BikeIcon },
...
export default function Sidebar({ active, onNav, collapsed, onToggle }) {
  return (
    <aside
      className={`h-screen bg-white border-r border-gray-100 flex flex-col transition-all duration-300 ${collapsed ? "w-16" : "w-56"
        }`}
    >
      {/* Logo */}
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center rounded-xl transition-all group ${collapsed ? "justify-center px-2 py-3" : "gap-3 px-3 py-2.5"
      ...
      <div className="px-2 py-4 border-t border-gray-50 space-y-1">
        <button
          className={`w-full flex items-center text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all ${collapsed ? "justify-center px-2 py-3" : "gap-3 px-3 py-2.5"
            }`}
        >
          <LogOut size={18} />
        ...
        <button
          onClick={onToggle}
          className={`w-full flex items-center text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-all ${collapsed ? "justify-center px-2 py-3" : "gap-3 px-3 py-2.5"
            }`}
        >
